...
def extractContents(s):
    first = slice(0,len(s)//2)
    second = slice(len(s)//2,len(s)) 
...
def run_part1():
    ...
    for line in input_file.readlines():
        # Part 1
        comp1,comp2 = extractContents(line)
        common_chars = findCommonChars(comp1, comp2)
        for c in common_chars:
            total = total + convertCharToPriority(c)
...
def findBadge(str1, str2, str3):
    return list(set(str1.strip())&set(str2.strip())&set(str3.strip()))
...
def run_part2():
    ...
    for line in input_file.readlines():
        ...
        if len(group) == 3:
            common_chars = findBadge(group[0], group[1], group[2])
            for c in common_chars:
                total = total + convertCharToPriority(c)
            group = []
