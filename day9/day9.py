...
# knots on a two-dimensional grid. Then, by following a hypothetical series of motions (your puzzle input) 
...
# diagram, s again marks the starting position (which the tail also visited) and # marks other positions 
# the tail visited:
#
# ..##..
# ...##.
...
def recordTailLocation(tail, visited):
    visited.add((tail.x, tail.y))


def updateHeadLocation(head, dir):
    if dir == "U":
        head.y = head.y + 1
    elif dir == "D":
        head.y = head.y - 1
...
def moveTrailingPoint(cur_point, prev_point):

    if cur_point.y == prev_point.y: # Points on same row
        prev_point.x = prev_point.x - 1 if cur_point.x < prev_point.x else prev_point.x + 1

    elif cur_point.x == prev_point.x: # Points in same col
        prev_point.y = prev_point.y - 1 if cur_point.y < prev_point.y else prev_point.y + 1

    else: # Must be diagonal
        isAbove = cur_point.y > prev_point.y
        isRight = cur_point.x > prev_point.x

        prev_point.x = prev_point.x + 1 if isRight else prev_point.x - 1
        prev_point.y = prev_point.y + 1 if isAbove else prev_point.y - 1
...
def runPart1(moves):
    ...
    visited = set()
    recordTailLocation(t, visited)

    for move in moves:
       
        for _ in range(move.n):
            updateHeadLocation(h, move.dir)

            if not isTouching(h, t):
                moveTrailingPoint(h, t)
                recordTailLocation(t, visited)
    
    print("Part 1: n visited =", len(visited))

# --- Part Two ---
# A rope snaps! Suddenly, the river is getting a lot closer than you remember. The bridge is still there, but 
# some of the ropes that broke are now whipping toward you as you fall through the air!
#
...
def runPart2(moves):
    ...
    recordTailLocation(tail, visited)

    for move in moves:
        for _ in range(move.n):
            updateHeadLocation(head, move.dir)

            for idx in range(len(points) - 1):
                point = points[idx]
                trailing_point = points[idx+1]

                if not isTouching(point, trailing_point):
                    moveTrailingPoint(point, trailing_point)

            recordTailLocation(tail, visited)

    print("Part 2: n visited =", len(visited))


def parseMoveFromLine(line):
    dir,n = line.split()
    return Move(dir, n)


def main():
    intput_file = open("./day9_input.txt", "r")
    instructions = [parseMoveFromLine(line) for line in intput_file.readlines()] 
