...
def getResult(a, b):
    if scores[a] == scores[b]:
        return "DRAW"
    return "WIN" if (scores[a] + 1) % 3 == scores[b] % 3 else "LOSS"

def calcScorePart1(line):
    p1,p2 = line.split()
    res = getResult(p1,p2)
    return scores[res] + scores[p2]
...
def calcScorePart2(line):
    p1,res = line.split()
    if res == "Y": # Required Draw
        return scores["DRAW"] + scores[p1]
    elif res == "X": # Required Loss
        return 3 if scores[p1] == 1 else scores[p1] - 1 
    else: # Required Win
        score = scores[p1] + 1
        if score > 3:
            score = 1
...
def main():
    ...
    for line in input_file.readlines():
        total_score_pt1 = total_score_pt1 + calcScorePart1(line) 
        total_score_pt2 = total_score_pt2 + calcScorePart2(line)
        
    print("Part 1: Total Score =", total_score_pt1)
    print("Part 2: Total Score =", total_score_pt2)
