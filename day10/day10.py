...
# of cathode-ray tube screen and simple CPU that are both driven by a precise clock circuit. The clock 
# circuit ticks at a constant rate; each tick is called a cycle.
#
# Start by figuring out the signal being sent by the CPU. The CPU has a single register, X, which starts 
...
class ActiveInstruction:
    def __init__(self, instruction, current_cycle):
        if instruction == "noop":
            ...
            self.end_cycle = current_cycle + 1
            self.val = int(val)
...
class StrengthProcessingManager:
    def __init__(self):
        self.next_cycle = 20

    def isProcessingRequired(self, current_cycle):
        if current_cycle < self.next_cycle:
            return False
        self.next_cycle = self.next_cycle + 40
        return True

def processOutput(output):
    print("".join(output[0:40]))
    print("".join(output[40:80]))
    print("".join(output[80:120]))
    print("".join(output[120:160]))
    print("".join(output[160:200]))
    print("".join(output[200:240]))
...
def main():
    ...
    inst_manager = InstructionsManager(instructions)
    ...
    signalStrengthMan = StrengthProcessingManager()
    ...
    while cycle < 240:
        ...
        if current_inst == None:
            current_inst = ActiveInstruction(inst_manager.getNextInstruction(), cycle)

        # process signal strength if multiple of 20
        if signalStrengthMan.isProcessingRequired(cycle):
            strengths_sum = strengths_sum + (cycle * x)
        ...
        # end of cycle
        if current_inst.end_cycle == cycle:
            if current_inst.instruction == "addx":
                x = x + current_inst.val
            current_inst = None

    print("Part 1: Strengths Sum =", strengths_sum)
